...
import { Plus, Search, MoreVertical, Eye, Edit, Trash2, Phone } from "lucide-react"
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage school suppliers and vendors</p>
        </div>
        <Button onClick={() => router.push("/reports/suppliers/add")}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-3xl font-bold text-green-600">{suppliers.filter((s) => s.status === "active").length}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <p className="text-3xl font-bold">
              GH₵ {suppliers.reduce((sum, s) => sum + s.totalAmount, 0).toLocaleString()}
            </p>
          </CardContent>
        </Card>
      </div>
            ...
            <TableBody>
              {filteredSuppliers.map((supplier) => (
                <TableRow key={supplier.id}>
                  ...
                  <TableCell className="font-semibold">GH₵ {supplier.totalAmount.toLocaleString()}</TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => router.push(`/reports/suppliers/${supplier.id}`)}>
                          <Eye className="w-4 h-4 mr-2" />
                          View Details
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => router.push(`/reports/suppliers/${supplier.id}/edit`)}>
